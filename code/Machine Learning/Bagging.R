...
auc_num<- function(train,test) {
  ...
  auc_out <- calcAUC(predict.bag(treelist,newdata=test),test[,outcome])
...
if (length(args)==0) {
  stop("USAGE: Rscript Bagging.R --fold k --input file --output out.csv", call.=FALSE)
}
i<-1 
while(i < length(args))
{
...
for (i in 1:fold_num) {
  ...
  if (i != fold_num) {
    ...
    treelist <-lapply(1:ntree, FUN=function(iter) {
      samp <- samples[,iter]
      rpart(delinquency_Formula, trainings_set[samp,],
...
best_treelist <-lapply(1:ntree, FUN=function(iter) {
  samp <- samples[,iter]
  rpart(delinquency_Formula, new_data[samp,],
        control=rpart.control(cp = 0.001,minsplit = 1000,
                              minbucket = 1000,
                              maxdepth=5))})
...
pred_data <- data.frame(Id = te_data$ID , Probability = predict.bag(best_treelist, newdata = te_data))
