...
auc_num<- function(train,test) {
  ...
  fmodel <- randomForest(x=train[,delinquencyVars], y=train[,outcome], 
                         ntree=100, nodesize=7, importance=T)
  
  
  
  varImp <- importance(fmodel)
  selVars <- names(sort(varImp[,1], decreasing=T))[1:8]
  
  fsel <- randomForest(x=train[,selVars],y=train[,outcome], 
                       ntree=100, nodesize=7, importance=T)
  
  
  pred <- predict(fsel,newdata=test[,selVars],
          type = "prob")[,pos]
  auc_out <- calcAUC(predcol = pred,
                     outcol = test[,outcome])
...
if (length(args)==0) {
  stop("USAGE: Rscript RandomForest.R --fold k --input file --output out.csv", call.=FALSE)
}
i<-1 
while(i < length(args))
{
...
pos <- "1"
tr_data[,outcome] <- as.factor(tr_data[,outcome])
...
best_fmodel <- randomForest(x=new_data[,delinquencyVars], y=new_data[,outcome], 
                       ntree=100, nodesize=7, importance=T)

best_varImp <- importance(best_fmodel)

best_selVars <- names(sort(best_varImp[,1], decreasing=T))[1:8]

best_fsel <- randomForest(x=new_data[,best_selVars],y=new_data[,outcome], 
                     ntree=100, nodesize=7, importance=T)
...
pred_data <- data.frame(Id = te_data$ID , Probability = predict(best_fsel,newdata=te_data[,best_selVars],
                                                                type = "prob")[,pos])
